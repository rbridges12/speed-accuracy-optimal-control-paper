...
these results are more realistic than a pure motor noise model, which displays primarily symmetric velocity profiles \cite{signal_dependent_motor_noise}; they are also more realistic than a torque model, which shows poorer correlation with experimental data \cite{5}.
...
The target radius was varied from 0.03m to 0.04m, and the target distance was varied from \todo{? to ?}.
All other parameters were held constant across trials, and the same set of parameters was used for both the feedforward and MPC models. \todo{add parameters}.

Once all trials were complete, a least-squares fit was performed on the data from each model in order to determine the $a$ and $b$ parameters for a Fitts' Law line. These values are $a = 0.24504$ and $b = 0.19348$ for the feedforward model, and $a = 0.30379$ and $b = 0.13263$ for the feedback model. Fit lines for the feedforward and feedback models are shown in Figures \ref{fig:FittsLawSingle} and \ref{fig:FittsLawMPC}, respectively.   
...
Both models produce $a,b$ values that fall within the ranges of $a \in [0.0047, 0.5239]$ and $b \in [0.0393,0.1987]$ reported in \cite{fitts_law_exp_data}. 
The $R^2$ values of 0.807 for the feedforward model and 0.721 for the feedback model both indicate good agreement with Fitts' Law.
