...
cmake ~/opensim-workspace/opensim-core-source/dependencies -DCMAKE_INSTALL_PREFIX=~/opensim-workspace/opensim-core-dependencies-install/ -DSUPERBUILD_ezc3d=on -DOPENSIM_WITH_CASADI=on -DOPENSIM_WITH_TROPTER=on -DSUPERBUILD_casadi=on -DSUPERBUILD_ipopt=on -DSUPERBUILD_colpack=on
cmake . -LAH
