...
int OptSystem::objectiveFunc(SimTK::Vector const& newControls,
                             bool new_coefficients, SimTK::Real& f) const {
    osimModel.initSystem();
    OpenSim::ArmController* controller =
            static_cast<OpenSim::ArmController*>(&osimModel.getControllerSet()[0]);
...
double OptSystem::evaluate_cost_function(bool new_coefficients) const {
    ...
    OpenSim::Set<OpenSim::Muscle> const& muscleSet = osimModel.getMuscles();
        ...
        if (prank == 0) {
            ...
            if (_initialTime < 1e-10) {
                ...
                ofs << "endheader"
                    << "\n";
                ofs << "time ";
                OpenSim::Array<std::string> var_names =
                        osimModel.getStateVariableNames();
    ...
    OpenSim::ArmController* armController =
            static_cast<OpenSim::ArmController*>(&osimModel.getControllerSet()[0]);

    bool passedOverlap = false;
    // double initialTime = _ts.getState().getTime();
    double initialTime = _manager.getState().getTime();
    ...
    SimTK::Vec3 massCenter = osimModel.getBodySet().get("hand").getMassCenter();
    ...
    while (_manager.getState().getTime() < _finalTime) {

        nbr_timesteps++;
        // _ts.stepTo(_finalTime);
        _manager.integrate(_finalTime);
        st = _manager.getState();
        ...
        if (!new_coefficients) {
            if (prank == 0) {
                if (passedOverlap) {
                    ...
                    //    osimModel.getSimbodyEngine().getPosition(st,
                    //    osimModel.getBodySet().get("hand"), massCenter, position);
                    position = osimModel.getBodySet().get("hand").getPositionInGround(st);
                    for (int k = 0; k < 3; k++) {
                        of_hand << position[k] << "\n";
                    }
                }
    ...
    //       st, osimModel.getBodySet().get("hand"), massCenter, position);
    position = osimModel.getBodySet().get("hand").getPositionInGround(st);
      osimModel.getSimbodyEngine().getVelocity(
          st, osimModel.getBodySet().get("hand"), massCenter, velocity);
    // velocity = osimModel.getBodySet().get("hand").getVelocityInGround(st);

    double pro_sup_angle = modelCoordinateSet[15].getValue(st);
    double pro_sup_diff = pow(pro_sup_angle - 1.57, 2);
    double wrist_diff = pow(modelCoordinateSet[16].getValue(st), 2) +
                        pow(modelCoordinateSet[17].getValue(st), 2);

    double task_term = pow(position[0] - _rand_end_eff[0], 2) +
                       pow(position[1] - _rand_end_eff[1], 2) +
                       pow(position[2] - _rand_end_eff[2], 2);
    ...
    // Put weights on these terms if you want to reach the final pose with the
    // forearm in pronation
    double pro_sup_weight = 0;
    double wrist_weight = 0;

    // To make time a variable in the optimization, you can determine if the task
    // is achieved inside the loop above at each timestep, and break from the loop
    // if it does after updating the min_time variable. Then, include a cost of
    // time objective to variable f, e.g.:
    // double cost_of_time_weight = 100;
    // double cost_of_time = min_time;

    std::cout << "task_term: " << task_weight * task_term << std::endl;
    std::cout << "activation cost: " << activation_weight * activation_cost
              << std::endl;
    std::cout << "joint limits costs: " << jointLimits_weight * jointLimits_cost
              << std::endl;
    std::cout << "velocity term: " << velocity_weight * velocity_term
              << std::endl;
    std::cout << "hand position: " << position[0] << " and " << position[1]
              << " and " << position[2] << std::endl;
    std::cout << "target position: " << _rand_end_eff[0] << " and "
              << _rand_end_eff[1] << " and " << _rand_end_eff[2] << std::endl;
    ...
    double f = task_weight * task_term + activation_weight * activation_cost +
               jointLimits_weight * jointLimits_cost +
...
void OptSystem::storing_excitations(SimTK::Vector const& excitations) const {
    std::ofstream fo;
...
void OptSystem::storing_states(SimTK::State const& s) const {
    OpenSim::CoordinateSet& modelCoordinateSet = osimModel.updCoordinateSet();
