...
public:
    OptSystem(int numParameters, SimTK::State const& s, OpenSim::Model& aModel, OpenSim::Manager& manager, std::string filename, std::string filename_states, double finalTime, double window_interval, int KNOTS)
        : OptimizerSystem(numParameters), osimModel(aModel), _manager(manager), si(s), _filename(filename), _filename_states(filename_states), _finalTime(finalTime), _window_interval(window_interval), _isSpasticity(false), _G(0.), _velocity_threshold(0.) {
    ...
    OptSystem(OptSystem const& rhs)
        : osimModel(rhs.osimModel), _manager(rhs._manager), si(rhs.si), _sf(rhs._sf) {
    ...
    void set_model_file(std::string model_file) { _model_file = model_file; };

    int objectiveFunc(SimTK::Vector const& newControls, bool new_coefficients,
                      SimTK::Real& f) const;
    double evaluate_cost_function(bool newCoefficients) const;
    void storing_excitations(SimTK::Vector const& excitations) const;
    void storing_states(SimTK::State const& s) const;
    void store_hand_kinematics(SimTK::State const& s) const;
    ...
    void set_isFinal(bool isFinal) { _isFinal = isFinal; };

    SimTK::State get_final_state() { return _sf; };

    void set_bestSoFar(double bestSoFar) { _bestSoFar = bestSoFar; };
    ...
    std::vector<SimTK::Vector> get_control_list() { return _control_list; };

    std::vector<SimTK::State> get_state_list() { return _state_list; };
};
